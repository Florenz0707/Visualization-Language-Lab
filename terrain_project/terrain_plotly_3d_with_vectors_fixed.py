...
def geometry_to_3d_coords(
    geom_obj, lon_1d, lat_1d, elev_grid, max_points=5000, z_offset=0.0
):
    xs_all = []
    ys_all = []
    zs_all = []
    ...
    if gtype == "LineString" or gtype == "LinearRing":
        xs, ys, zs = process_coords(list(geom_obj.coords))
        append_coords(xs, ys, zs)
    elif gtype == "Polygon":
        ext = geom_obj.exterior
        xs, ys, zs = process_coords(list(ext.coords))
        append_coords(xs, ys, zs)
        for interior in geom_obj.interiors:
            xs, ys, zs = process_coords(list(interior.coords))
            append_coords(xs, ys, zs)
    elif gtype.startswith("Multi") or gtype == "GeometryCollection":
        for part in geom_obj.geoms:
            xsub, ysub, zsub = geometry_to_3d_coords(
                part,
                lon_1d,
                lat_1d,
                elev_grid,
        ...
        try:
            ...
            xs, ys, zs = process_coords(coords)
            append_coords(xs, ys, zs)
        except Exception:
            pass
...
def render_3d_with_vectors(
    ...
    terrain = (
        build_mesh(lon, lat, elev_plot)
        if mode == "mesh"
        else build_surface(lon, lat, elev_plot)
    )
    data = [terrain]
    ...
    elev_range = (
        np.nanmax(elev_plot) - np.nanmin(elev_plot)
        if np.isfinite(elev_plot).any()
        else 1.0
    )
    ...
    if g_countries is not None and len(g_countries) > 0:
        ...
        for geom1 in g_countries.geometry:
            xsub, ysub, zsub = geometry_to_3d_coords(
                geom1, lon_1d, lat_1d, elev_plot, max_points=3000, z_offset=z_offset
            )
            if xsub:
                xs.extend(xsub)
                ys.extend(ysub)
        ...
        if cx:
            data.append(
                go.Scatter3d(
                    ...
                    z=cz,
                    mode="markers",
                    marker=dict(size=sizes, color="red", opacity=0.8),
                    hovertext=text,
                    hoverinfo="text",
                    name="Cities",
                )
...
def main(argv):
    p = argparse.ArgumentParser(
        description="3D Terrain + Vector Overlays (fixed lon/lat handling)"
    )
    p.add_argument(
        "--file",
        type=str,
        required=True,
        help="输入 GeoTIFF (DSM)（例如 merged_dem_cropped.tif）",
    )
    p.add_argument(
        "--out", type=str, default="terrain3d_with_vectors.html", help="输出 HTML 文件"
    )
    p.add_argument(
        "--render",
        type=str,
    ...
    p.add_argument(
        "--z-exag", type=float, default=1.0, help="Z 轴放大倍数（在 auto-z 开启时为额外放大）"
    )
    p.add_argument("--static", action="store_true", help="输出静态 HTML（禁用交互）")
    p.add_argument(
        "--countries",
        type=str,
        default=r"F:/Professional_books/visualization/project/Visualization-Language-Lab/terrain_project/public/data/boundaries/ne_10m_admin_0_countries/ne_10m_admin_0_countries.shp",
        help="admin_0 shapefile",
    )
    p.add_argument(
        "--admin1",
        type=str,
        default=r"F:/Professional_books/visualization/project/Visualization-Language-Lab/terrain_project/public/data/boundaries/ne_10m_admin_1_states_provinces/ne_10m_admin_1_states_provinces.shp",
        help="admin_1 shapefile",
    )
    p.add_argument(
        "--cities",
        type=str,
        default=r"F:/Professional_books/visualization/project/Visualization-Language-Lab/terrain_project/public/data/boundaries/ne_10m_populated_places/ne_10m_populated_places.shp",
        help="populated places shapefile",
    )
    p.add_argument(
        "--rivers",
        type=str,
        default=r"F:/Professional_books/visualization/project/Visualization-Language-Lab/terrain_project/public/data/boundaries/ne_10m_rivers_lake_centerlines/ne_10m_rivers_lake_centerlines.shp",
        help="rivers shapefile",
    )
    ...
    p.add_argument(
        "--smooth-sigma", type=float, default=1.0, help="gaussian sigma（像素单位）"
    )
    ...
    print(
        "lon shape:",
        getattr(lon, "shape", None),
        "lat shape:",
        getattr(lat, "shape", None),
    )
    ...
    # load shapefiles
    g_countries = (
        load_shapefile(args.countries, simplify_tol=args.simplify_tol)
        if args.countries
        else None
    )
    g_admin1 = (
        load_shapefile(args.admin1, simplify_tol=args.simplify_tol)
        if args.admin1
        else None
    )
    g_cities = (
        load_shapefile(args.cities, simplify_tol=args.simplify_tol)
        if args.cities
        else None
    )
    g_rivers = (
        load_shapefile(args.rivers, simplify_tol=args.simplify_tol)
        if args.rivers
        else None
    )

    render_3d_with_vectors(
        lon,
        lat,
        elev_plot,
        args.out,
